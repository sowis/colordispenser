...
function magnification(mouse_y, mouse_x) {
    ...
    for (let offset_y = 0; offset_y < setting.magnifier_pixel; ++offset_y) {
        for (let offset_x = 0; offset_x < setting.magnifier_pixel; ++offset_x) {
            const original_y = mouse_y - (setting.magnifier_pixel / 2) + offset_y;
            const original_x = mouse_x - (setting.magnifier_pixel / 2) + offset_x;
            ...
            for (let magnifier_y = offset_y * ($magnifier.height / setting.magnifier_pixel); magnifier_y < (offset_y + 1) * ($magnifier.height / setting.magnifier_pixel); ++magnifier_y) {
                for (let magnifier_x = offset_x * ($magnifier.width / setting.magnifier_pixel); magnifier_x < (offset_x + 1) * ($magnifier.width / setting.magnifier_pixel); ++magnifier_x) {
